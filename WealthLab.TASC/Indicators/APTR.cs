...
namespace WealthLab.TASC
{
    public class APTR : IndicatorBase
    {
        ...
        //static method
        public static APTR Series(BarHistory source, int period)
        {
            string key = CacheKey("APTR", period);
            if (source.Cache.ContainsKey(key))
                return (APTR)source.Cache[key];
            APTR aptr = new APTR(source, period);
            source.Cache[key] = aptr;
            return aptr;
        }
        ...
        public override void Populate()
        {
            ...
            WilderMA wma = new WilderMA(atrs_, 14);
            for (int bar = period + 2; bar < bars.Count; bar++)
            {
                Values[bar] = wma[bar] * 100;
            }
        }        
    }
}