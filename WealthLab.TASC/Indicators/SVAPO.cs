...
namespace WealthLab.TASC
{
    public class SVAPO : IndicatorBase
    {
        ...
        //static method
        public static SVAPO Series(BarHistory source, int period, double cutoff)
        {
            string key = CacheKey("SVAPO", period, cutoff);
            if (source.Cache.ContainsKey(key))
                return (SVAPO)source.Cache[key];
            SVAPO s = new SVAPO(source, period, cutoff);
            source.Cache[key] = s;
            return s;
        }
        ...
        protected override void GenerateParameters()
        {
            ...
            AddParameter("Period", ParameterType.Int32, 8);
            AddParameter("Cutoff", ParameterType.Double, 1.0);
        }
