...
namespace WealthLab.TASC
{
    public class FourierCTS : IndicatorBase
    {
        ...
        //static method
        public static FourierCTS Series(TimeSeries source, int period)
        {
            string key = CacheKey("FourierCTS", period);
            if (source.Cache.ContainsKey(key))
                return (FourierCTS)source.Cache[key];
            FourierCTS fc = new FourierCTS(source, period);
            source.Cache[key] = fc;
            return fc;
        }
        ...
        protected override void GenerateParameters()
        {
            ...
            AddParameter("Fundamental cycle period", ParameterType.Int32, 20);
        }
    }
}