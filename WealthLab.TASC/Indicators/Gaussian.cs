...
namespace WealthLab.TASC
{
    public class Gaussian : IndicatorBase
    {
        ...
        //static method
        public static Gaussian Series(TimeSeries source, int period, int poles)
        {
            string key = CacheKey("Gaussian", period, poles);
            if (source.Cache.ContainsKey(key))
                return (Gaussian)source.Cache[key];
            Gaussian g = new Gaussian(source, period, poles);
            source.Cache[key] = g;
            return g;
        }
        ...
        protected override void GenerateParameters()
        {
            ...
            AddParameter("poles", ParameterType.Int32, 3);
