...
namespace WealthLab.TASC
{
    public class CorrelationAngle : IndicatorBase
    {
        ...
        //static method
        public static CorrelationAngle Series(TimeSeries source, int period = 14)
        {
            string key = CacheKey("CorrelationAngle", period);
            if (source.Cache.ContainsKey(key))
                return (CorrelationAngle)source.Cache[key];
            CorrelationAngle ca = new CorrelationAngle(source, period);
            source.Cache[key] = ca;
            return ca;
        }
