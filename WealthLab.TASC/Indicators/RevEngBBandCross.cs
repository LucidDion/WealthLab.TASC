...
namespace WealthLab.TASC
{
    ...
    for Bar := BarCount - 1 downto Period  - 2 do
      @Result[Bar] := @A[Bar] - k * @SD[Bar];         
        ...
        //static method
        public static RevEngBBandCross Series(TimeSeries source, int period, double devs, bool upper)
        {
            string key = CacheKey("RevEngBBandCross", period, devs, upper);
            if (source.Cache.ContainsKey(key))
                return (RevEngBBandCross)source.Cache[key];
            RevEngBBandCross rebbc = new RevEngBBandCross(source, period, devs, upper);
            source.Cache[key] = rebbc;
            return rebbc;
        }
        ...
        protected override void GenerateParameters()
        {
            ...
            AddParameter("Deviations", ParameterType.Double, 2);
            AddParameter("Upper?", ParameterType.Boolean, true);
        }
        ...
        public override void Populate()
        {
            ...
            var devs2 = devs * devs;
            if (!Upper) upper = -1;
            var _smaSer = FastSMA.Series(ds, period);
            var _sdSer = StdDev.Series(ds, period);
