...
namespace WealthLab.TASC
{
    ...
    public class FVE : IndicatorBase
    {
        ...
        //static method
        public static FVE Series(BarHistory source, int period)
        {
            string key = CacheKey("FVE", period, period);
            if (source.Cache.ContainsKey(key))
                return (FVE)source.Cache[key];
            FVE fve = new FVE(source, period);
            source.Cache[key] = fve;
            return fve;
        }
