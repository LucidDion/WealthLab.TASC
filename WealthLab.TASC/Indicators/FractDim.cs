...
namespace WealthLab.TASC
{
    ...
    public class FractDim : IndicatorBase
    {
        ...
        //static method
        public static FractDim Series(TimeSeries source, int period)
        {
            string key = CacheKey("FractDim", period);
            if (source.Cache.ContainsKey(key))
                return (FractDim)source.Cache[key];
            FractDim fd = new FractDim(source, period);
            source.Cache[key] = fd;
            return fd;
        }
        ...
        public static double Value(int bar, TimeSeries ds, int period)
        {
            ...
            // Calculate length
            double Range = Highest.Value(bar, ds, period) - Lowest.Value(bar, ds, period);
            if (Range <= 0) return 0;
