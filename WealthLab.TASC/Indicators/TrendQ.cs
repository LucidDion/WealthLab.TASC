...
namespace WealthLab.TASC
{
    public class TrendQ : IndicatorBase
    {
        ...
        //static method
        public static TrendQ Series(TimeSeries source, int period1, int period2, int m, int n, int c, bool UseRMSNoise)
        {
            string key = CacheKey("TrendQ", period1, period2, m, n, c, UseRMSNoise);
            if (source.Cache.ContainsKey(key))
                return (TrendQ)source.Cache[key];
            TrendQ tq = new TrendQ(source, period1, period2, m, n, c, UseRMSNoise);
            source.Cache[key] = tq;
            return tq;
        }
