...
namespace WealthLab.TASC
{
    public class HACOLT : IndicatorBase
    {
        ...
        //static method
        public static HACOLT Series(BarHistory source, int period, double factor, int timeout, int average)
        {
            string key = CacheKey("HACOLT", period);
            if (source.Cache.ContainsKey(key))
                return (HACOLT)source.Cache[key];
            HACOLT h = new HACOLT(source, period, factor, timeout, average);
            source.Cache[key] = h;
            return h;
        }
        ...
        protected override void GenerateParameters()
        {
            ...
            AddParameter("Candle Size factor", ParameterType.Double, 1.1);
            AddParameter("Setup timeout", ParameterType.Int32, 2);
            AddParameter("Shorting LT Average", ParameterType.Int32, 60);
        }
