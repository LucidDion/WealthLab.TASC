...
namespace WealthLab.TASC
{
    public class MyRSI : IndicatorBase
    {
        ...
        public MyRSI(TimeSeries source, int period, int k)
            : base()
        {
        ...
        //static method
        public static MyRSI Series(TimeSeries source, int period, int k)
        {
            string key = CacheKey("MyRSI", period);
            if (source.Cache.ContainsKey(key))
                return (MyRSI)source.Cache[key];
            MyRSI mr = new MyRSI(source, period, k);
            source.Cache[key] = mr;
            return mr;
        }
        ...
        protected override void GenerateParameters()
        {
            ...
            AddParameter("Smoothing Length", ParameterType.Int32, 8);
            AddParameter("RSI Length", ParameterType.Int32, 10);
        }
